// POST /api/admin/photos/:id  with body { action: 'hide' | 'unhide' | 'approve' }
//
...
//
// 'approve' promotes a 'pending' photo (moderation-mode hit) to 'visible',
// mirroring danmaku's approve action.
...
export const onRequestPost: PagesFunction<Env> = async ({ request, env, params }) => {
  ...
  if (action !== 'hide' && action !== 'unhide' && action !== 'approve') {
    return err(400, 'action must be hide, unhide, or approve')
  }
  const newStatus = action === 'hide' ? 'hidden' : 'visible'

  // Unhide and approve both bump created_at to now: the screen's feed
  // cursor has long moved past the original timestamp, so without the bump
  // the photo would never (re-)enter the carousel.
  const res = action === 'hide'
    ? await env.DB
